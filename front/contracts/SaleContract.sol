...
contract SaleContract {
    ...
    function ListFotSaleContract(uint8 _tokenId, uint _price) public {
        ...
        // 판매 리스트 배열에 추가
        SaleContractList.push(_tokenId);
    ...
    function popSaleContract(uint8 _tokenId) private returns (bool) {
        for (uint256 i = 0; i < SaleContractList.length; i++) {
            if(SaleContractList[i] == _tokenId){
                ...
                SaleContractList[i] = SaleContractList[SaleContractList.length - 1];

                // 배열의 마지막 요소 삭제
                SaleContractList.pop();
                return true;
            }
        }
        return false;
    ...
    // 전체 판매 리스트 확인 view
    function getSaleContractList() public view returns (TokenInfo[] memory){
        
        // 판매 리스트가 한 개 이상일 경우에만 실행
        require(SaleContractList.length > 0);
